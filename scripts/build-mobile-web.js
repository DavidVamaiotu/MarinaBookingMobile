...
function buildMobileDefines(environment = process.env) {
  const config = MarinaConfig.createConfig(environment);
  const publicEnvironment = MarinaConfig.publicEnvironment(config);
  return Object.fromEntries(MarinaConfig.PUBLIC_CONFIG_KEYS.map((name) => [
    `__${name}__`,
    JSON.stringify(publicEnvironment[name])
  ]));
}
...
if (require.main === module) buildMobileWeb();

module.exports = { buildMobileDefines, buildMobileWeb };
